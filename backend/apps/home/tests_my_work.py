...
class ChangesOnYourItems(TestCase):
    ...
    def test_a_finished_case_is_not_a_change_on_your_items(self) -> None:
        change, case = self.linked_case()
        self.confirm(change, confirmed_by=self.bank.officer)
        # A dismissal names its reason (change_case_dismissal_needs_a_reason).
        ChangeCase.objects.filter(pk=case.pk).update(
            status=CaseStatusCategory.DISMISSED.value, dismissed_reason=DismissalReason.objects.get(key="out_of_scope")
        )
