...
class RegisteringAReform(RegistrationCase):
    ...
    def test_every_suggestion_names_the_run_agent_and_key_that_filed_it(self) -> None:
        ...
        confirming it (D-74). A library editor filing by hand names no run, so no agent."""
        obligation = self.an_obligation()
        self.register(
            body(
                agentRunId=str(self.open_run.id),
        ...
        change = self.stored()
        suggester = (self.open_run.agent_id, self.open_run.api_key_id)
        self.assertEqual((change.change_type_suggested_by_agent_id, change.change_type_suggested_by_api_key_id), suggester)
        ...
        for link in links:
            with self.subTest(link=str(link)):
                self.assertEqual((link.suggested_by_agent_id, link.suggested_by_api_key_id), suggester)
        ...
        self.assertIsNone(by_hand.change_type_suggested_by_agent_id)
        self.assertTrue(by_hand.change_type_suggested, "a person's filing is a suggestion too")
        self.assertEqual({link.suggested_by_agent_id for link in by_hand.term_links.all()}, {None})
