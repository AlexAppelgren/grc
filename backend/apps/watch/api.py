...
def update_change(
    ...
    **A drafted “So what?” may be filed here too** (D-66, WAT-05): send `soWhat`
    ...
    never from a tag;
    `regime_required` (422) when `termIds` replaces the set with one that names no regime
    term, because every change carries a regime (D-39), with the regime keys in `validKeys`;
    `standard_term_only_on_standards` (422) when that set carries a standard's term (a term
    of an opt-in dimension) and the change's authority is not a standards body, whose
    jurisdiction is international;
    `editor_only_field` (422) when a key sends `status` or
