...
class IdentityLookupMode(TestCase):
    ...
    def test_only_record_leaves_the_tenant_zone(self) -> None:
        ...
        `apps/search/indexing.py` is the third door and was reviewed as one (owner item 4):
        a search chunk is derived data, every chunk in R1 belongs to no tenant, and the
        words in one are copied from a shared library record that `apps/search/sources.py`
        refuses to read if a bank owns it. Nothing a bank wrote can ride out of its zone
        this way, and no other module may write a chunk at all
