...
def approve_proposal(
    ...
    cannot name its confirming agent, which waits for a person (no kind today);
    ...
    not hold; `jurisdiction_term_mirrored` (422) when the payload adds or renames a term of
    a dimension that mirrors the jurisdiction list, or scopes an obligation with one, which
    a proposal filed before that rule may still ask for.
    """
    reviewer = require_reviewer(request)
