...
def approve_proposal(
    ...
    it. An agent approves every kind whose record can name the agent that confirmed it,
    which today is every kind: an obligation version, a library list row and a taxonomy
    term (D-79). What it confirmed never reads as a person's check: the record names the
    confirming agent, a translation it approves stays labelled machine-made, every list or
    term label it writes is stored machine-made, and its correction may reword a summary
    but never move `originalLanguage`, which answers `validation_error`.
    ...
    the owner has not opened to an agent (a new instrument, obligation or provision, or a
    provision version), which waits for a person;
    `invalid_transition` when the proposal was already approved or rejected, which is also
    what a repeated or simultaneous second call answers, since nothing is ever applied
    twice; `source_missing` when a correction introduces a field the proposal never
    sourced; `validation_error` when a key sends no `decision` or a person sends one or
    names a run, and when a correction is offered on a kind that cannot be corrected or
    does not fit its payload; `unknown_key` when the payload names a row the library does
    not hold; `not_a_regime` when a new instrument's regime is not a term of the regime
    dimension; `duplicate_key` when a new record's key was taken while the proposal
    waited; `jurisdiction_term_mirrored` (422) when the payload adds or renames a term of
    a dimension that mirrors the jurisdiction list, or scopes an obligation with one,
    which a proposal filed before that rule may still ask for;
