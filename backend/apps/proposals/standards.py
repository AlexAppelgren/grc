...
- `licensed_text`: no provision and no provision version under a standard, and every field
...
def check(
    kind: str, instrument: Instrument, term_ids: Collection[uuid.UUID] | None, field_sources: Mapping[str, str]
) -> None:
...
def check_payload(
    ...
    if isinstance(payload, ProposalObligationVersionPayload) and target_id is not None:
        ...
        instrument, terms = shared_instrument(payload.instrument), payload.terms or []
    ...
    check(kind, instrument, term_ids, field_sources)
