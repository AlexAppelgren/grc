...
class ProposalRow(CamelSchema):
    ...
    id: UUID = Field(description="The proposal, as the console addresses it. Server-assigned and stable.")
    kind: str = Field(
        description=(
    ...
    target_id: UUID | None = Field(default=None, description="The library record the proposal changes, or null when it creates one. Read the record itself for what it currently says.")
    change_id: UUID | None = Field(default=None, description="The regulatory change that prompted this, when an agent's watch run found one. Null means nobody linked one, not that no change exists.")
    ...
    agent_run_id: UUID | None = Field(default=None, description="The agent run that produced the proposal, for the run log. Null for a person's proposal.")
    ...
    reviewed_at: datetime | None = Field(default=None, description="When the decision was made, in UTC. Null while the proposal is open.")
    ...
    applied_at: datetime | None = Field(default=None, description="When the change reached the library, in UTC, which is the moment of approval. Null unless the status is `approved`.")
    created_at: datetime = Field(description="When the proposal was filed, in UTC. Not the legal date of the change, which is `effectiveFrom`.")
