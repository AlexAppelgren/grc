...
5. **Embedding happens out of band, through the one relay.** The outbox cursor delivers
   the library event with no tenant active, and the handler fills the embeddings in
   batches. A failing embedder costs its own row an attempt and nothing else.
"""
...
class TheEmbeddingRelay(LibraryFixtureMixin, TestCase):
    ...
    def test_the_handler_is_registered_for_the_events_a_rebuild_emits(self) -> None:
        for topic in tasks.INDEX_TOPICS:
            with self.subTest(topic=topic):
                self.assertIn(tasks.embed_rebuilt_chunks, outbox._HANDLERS.get(topic, []))
    ...
    def test_a_batch_smaller_than_the_backlog_still_embeds_all_of_it(self) -> None:
        with transaction.atomic():
