...
class SimilarRequest(WriteBody):
    ...
    types: list[SearchHitType] = Field(
        default_factory=list,
        description=(
            "Which kinds of record to compare against; empty means all three. Source: the "
            "calling agent. Do not read an empty list as a promise of three kinds in the "
            "results."
        ),
    )
    limit: int = Field(
...
class Answer(CamelSchema):
    ...
    created_at: datetime = Field(
        description=(
            "When the answer was produced, in UTC. Source: the server. Do not read it as "
            "the date the law was read at, which is `asOf`."
        )
    )
...
class AskProblemEvent(CamelSchema):
    ...
    code: str = Field(
        description=(
            ...
            "problem body would carry, such as `not_built`, `feature_off` or "
            "`rate_limited`. Branch on this, never on `detail`. Source: the server. Do not "
            "read a code as a permanent verdict: `rate_limited` clears within the minute."
        )
    )
    detail: str = Field(
        description=(
