...
def member_out(membership: Membership, order: list[str], *, passkey_count: int, active_sessions: int) -> dict[str, Any]:
...
def list_members(tenant_id: uuid.UUID, order: list[str], *, limit: int, offset: int) -> tuple[list[dict[str, Any]], int]:
    ...
    return [member_out(m, order, passkey_count=m.passkey_count, active_sessions=m.active_sessions) for m in rows], total
...
def member_detail(tenant_id: uuid.UUID, user_id: uuid.UUID, order: list[str]) -> dict[str, Any]:
    ...
    return member_out(membership, order, passkey_count=passkeys, active_sessions=sessions)
