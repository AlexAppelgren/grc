...
a second run the same day is silent. A person who muted reminders gets no row and no mail.
...
def send_triage_reminders(tenant: Tenant) -> list[Notification]:
    ...
    cases = list(
        ChangeCase.objects.filter(due, status=CaseStatusCategory.NEW.value).select_related("change").order_by("id")
    )
    if not cases:
        return []
    already = set(
        EmailMessage.objects.filter(
            sent_on=today, subject_type=SUBJECT_TYPE, subject_id__in=[case.id for case in cases]
...
def _mail(sends: list[tuple[Notification, mail.Template, mail.MailContext]]) -> None:
    """Queue one mail per notification and stamp the rows that were mailed, in one pass."""
    if not sends:
        return
    members = {
        membership.user_id: membership
        for membership in Membership.objects.filter(user_id__in={row.user_id for row, _t, _c in sends}).select_related(
            "user__locale"
        )
    }
