...
def create_cases(event: OutboxEvent) -> None:
    ...
    for tenant_id in _active_tenant_ids():
        _create_case(tenant_id, facts=facts, restricting=restricting)


def _active_tenant_ids() -> list[uuid.UUID]:
    """Every active bank, read once and in full before the first case is written.
    ...
    """
    queryset = Tenant.objects.filter(status=TenantStatus.ACTIVE.value).order_by("slug").values_list("id", flat=True)
    return list(queryset.iterator(chunk_size=settings.CASE_CREATION_BATCH))


@tenancy.tenant_task
def _create_case(tenant_id: uuid.UUID, *, facts: reading.ChangeFacts, restricting: Collection[str]) -> None:
