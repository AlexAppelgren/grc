...
describe('footprint api', () => {
  ...
  it('lists terms by the key of their dimension, and the dimensions with their footprint flag', async () => {
    const term = { id: 't1', key: 'advice', label: 'Advice', dimension: { key: 'service_type', kind: null, label: 'Services we provide' }, active: true, isSystem: true, sortOrder: 0, usageNote: '', version: 1 };
    const dimensions = [
