...
import { useCreateValue, useMergeValue, usePreviewMerge, useReorderValues, useRetireValue, useSuggestValue, useUpdateValue, useVocabularies, useVocabularyValues, vocabularyKeys } from './hooks';
...
describe('vocabularies hooks', () => {
  ...
  it('writes through every mutation and refreshes the list afterwards', async () => {
    const sent = installAdapter((s) => ({ status: s.method === 'get' ? 200 : s.path.endsWith('/suggest') ? 201 : 200, data: s.method === 'get' ? [] : s.path.endsWith('/merge') ? { from: 'a', into: 'b', repointed: 2, usageCount: 2, dryRun: false } : { key: 'a', proposal: { id: 'p1' } } }));
    const { wrapper } = queryWrapper();
    ...
    expect(await renderHook(() => useSuggestValue('tenant_tag'), { wrapper }).result.current.mutateAsync({ labels: { en: 'C' } })).toEqual({ id: 'p1', kind: '', status: '', title: '' });
