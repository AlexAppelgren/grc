...
import { metadata } from '@/app/(auth)/invite/[token]/page';
...
import { InvitationScreen, replaceWithEnrol } from './InvitationScreen';
...
describe('the invitation code step', () => {
  ...
  it('asks for the code alone and verifies it with the token held in memory, never in the URL or storage', async () => {
    ...
    const sent = server();
    renderIn(<InvitationScreen token={TOKEN} />);
    ...
    expect(sent.filter((s) => s.path !== `/api/v1/auth/invitations/${TOKEN}/open`).map((s) => s.path).join(' ')).not.toContain(TOKEN);
  ...
  it('says a wrong code is not right, and "Send a new code" opens the same invitation again', async () => {
    ...
    expect(sent.filter((s) => s.path === `/api/v1/auth/invitations/${encodeURIComponent(TOKEN)}/open`)).toHaveLength(2);
  });
  ...
  it('shows the expired and error states when the link cannot be opened', async () => {
    ...
    const view = renderIn(<InvitationScreen token={TOKEN} />);
    await screen.findByText('This invitation is no longer valid. Ask your administrator to send a new one.');
