...
test.describe('shared journeys', () => {
  ...
  // The catalog half of I18N-S3 is `npm run check:messages`, which fails on a key
  // missing in either language and on a string literal in JSX text
  // (scripts/messages-check.test.mjs). This is the person's half: the interface
  // switched to Swedish and back from the account menu, the vocabulary labels
  // coming from their Swedish rows. The login is its own (reserved for I18N-S3),
  // because the language is saved on the person and every session of theirs follows it.
  test('I18N-S3: Every UI string is in the catalog for every shipped language', async ({ page, apiGuard }) => {
    ...
    const svenska = page.getByRole('menuitemradio', { name: /^Svenska$/ });
    ...
    try {
      ...
      // Teardown that runs on failure too: the login goes back to English. A fresh
      // load closes whatever was open; choosing the language in use saves nothing.
      await page.goto('/');
      await accountMenu.click();
      await english.click();
      await expect(english).toHaveAttribute('aria-checked', 'true');
  ...
  test('I18N-S4: Dates, numbers and partial dates format per language and timezone', async ({ page, apiGuard }) => {
    ...
    // The journeys run in one pinned language: the document keeps the build
    // default while this person's screens read Swedish.
