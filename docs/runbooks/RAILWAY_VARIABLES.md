...
| `CASE_ACTIONS_MAX` | api | `200` | `200` | `200` | How many live actions one case may carry (CAS-04). One more answers 409 `too_many_actions`; a removed action no longer counts. It keeps the case file and the sign-off guard, which read every action of a case, inside the 250 ms budget |
...
| `REGISTER_BULK_MAX` | api | `100` | `100` | `100` | The most applicability answers one confirmed call stores, such as the decisions of a pasted Statement of Applicability (REG-01, AC-REG1, D-75). Every row is a write and an audit event in one transaction, so the cap keeps a call inside the API budget; a longer call answers 422 and stores nothing |
