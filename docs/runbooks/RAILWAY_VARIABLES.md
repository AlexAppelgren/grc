...
| `REMINDER_SEND_HOUR` | worker | `7` | `7` | `7` | The hour, 0 to 23 in each bank's own time zone, at which the day's reminders before and after a due date go out (COL-02). Beat runs the job every hour and it picks the banks whose own clock has just reached this hour, so a daylight saving change moves the UTC hour and never the local one |
...
| `MY_WORK_DUE_SOON_DAYS` | api | `30` | `30` | `30` | How far ahead My work's "Due soon" section reaches (HOM-05, D-23): a row whose next date is the bank's own today or within this many days is due soon, and one further out is under "Everything you're responsible for". Replaced by the bank's reminder lead once COL-02 lands. At least 1, or the app refuses to boot |
| `MY_WORK_AWARE_DAYS` | api | `14` | `14` | `14` | How many days a new version of an obligation stays under My work's "Changes on your items" after it was applied (HOM-05, D-25). A fixed window needs no write on every page load. At least 1, or the app refuses to boot |
| `DIGEST_MAX_ITEMS` | worker | `20` | `20` | `20` | How many of a person's My work rows the weekly digest mail lists (COL-02, HOM-05): the most urgent ones, then "and N more", while the section counts still cover every row. 1 to 100, or the app refuses to boot |
| `DIGEST_SEND_HOUR` | worker | `7` | `7` | `7` | The hour, 0 to 23 in each bank's own time zone, at which the weekly digest goes out on the bank's `digest_weekday` (COL-02). The beat runs hourly and hands on the banks whose clock has just struck it, so a daylight saving change moves the UTC hour, never the local one; keep it off 02 to 03, which such a change skips or repeats. Outside 0 to 23 the app refuses to boot |
...
| `COMMENT_EDIT_MINUTES` | api | `15` | `15` | `15` | How many minutes after writing a comment its author may still correct it (COL-01). Afterwards it can be deleted but not edited (409 `edit_window_closed`), so colleagues never find a comment rewritten under them long after they read it. Every edit keeps the text it replaced |
