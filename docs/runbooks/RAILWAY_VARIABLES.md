...
| `SEARCH_BUDGET_MS` | api | `800` | `800` | `800` | Hybrid search's budget without the reranker, in `Server-Timing` (NFR-02). The search tests hold the route to it, and a search row in `backend/perf/routes.py` names it as its budget |
| `SEARCH_RERANKED_BUDGET_MS` | api | `1500` | `1500` | `1500` | The same with the reranker on (NFR-02) |
| `ASK_FIRST_TOKEN_BUDGET_MS` | api | `2000` | `2000` | `2000` | Ask's budget: the time to the first streamed chunk, not to the whole answer (NFR-02, playbook 10). The performance harness times a stream to that chunk, and the Ask row in `backend/perf/routes.py` names this as its budget |
