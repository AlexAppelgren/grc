...
| `WATCH_RUN_MAX_PROPOSALS` | api | `50` | `50` | `50` | The most proposals one agent run may file (AGT-01, H24); the next answers 422 `run_budget_exhausted` and stores nothing. At least 1, or the app refuses to boot |
| `WATCH_RUN_MAX_CHANGES` | api | `50` | `50` | `50` | The most new changes one agent run may register on the watch feed (AGT-01, H24); a second sighting of a change it already holds is not counted. At least 1, or the app refuses to boot |
| `WATCH_RUN_MAX_RECHECKS` | api | `100` | `100` | `100` | The most re-check lines one agent run may log in the coverage log (WAT-01, H24); a sweep of a source is not counted. At least 1, or the app refuses to boot |
