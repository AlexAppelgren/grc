...
| `AGENT_ACCESS_DESCRIPTION_MAX_CHARS` | api | `2000` | `2000` | `2000` | The longest description `POST /agent-access/what-applies` takes, in characters; a longer one answers 422 `description_too_long` (ACC-06). At least 1 or the app refuses to boot |
| `AGENT_ACCESS_NOTE_MATCH_WORDS` | api | `2` | `2` | `2` | How many words a footprint term's usage note must share with a what-applies description for the answer to name that term as outside the entry's scope (ACC-07); a label matches when all its words are in the description. At least 1 or the app refuses to boot |
