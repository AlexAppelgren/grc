# ADR 0054 — The second pair of eyes on a library proposal may be an independent agent

**Date:** 2026-09-20 · **Status:** accepted (owner decision, item 19; D-62, PRD 0.4 PRO-01, PRO-02, INV-05, AUD-02)
...
- **The constraint.** `proposal` gains `reviewed_by_api_key`, `proposed_by_agent`
  ...
  **or** agent is the same on both sides, so two keys of one agent definition
  cannot confirm each other. It also refuses a reviewing key that names no agent
  (`reviewed_by_api_key IS NULL OR reviewed_by_agent IS NOT NULL`), because an
  unbound key would carry a NULL agent, and NULL never equals NULL: two such keys
  would pass the agent comparison on a missing value rather than on independence.
  The database, not Python, stays the word on four eyes.
