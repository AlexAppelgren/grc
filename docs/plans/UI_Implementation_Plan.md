...
| `admin-agent-access.html` | `/admin/agents/access`, `/admin/agents/access/[entryId]` | tenant | 11 | built (acc-fe-admin), designed (acc-cards): the Access tab with its read-only line and the reach line; entries with state, reach and scope pills; register and edit with department, product and team pickers and a step-up; revoke; the entry page with credentials (issue with expiry at most 90 days, shown once, last use, revoke), the per-entry reach toggle disabled with its reason while the organisation's switch is off, and the access log with no content (ACC-S1, ACC-S3, ACC-S6, ACC-S12, J-11) |  |
...
| Operation | Card | Surface | Permission | Step-up | Role variants | E2E | Status |
|---|---|---|---|---|---|---|---|
| `GET /agent-access`, `GET /agent-access/{id}` | `admin-agent-access.html` list and entry page | tenant | `agent_access.manage` | no | A; the tab is not offered without it, another tenant's entry is 404 | ACC-S1, ACC-S13 | built (acc-fe-admin) |
| `POST /agent-access` (register) | `admin-agent-access.html` Register an agent | tenant | `agent_access.manage` | yes | A | ACC-S1, ACC-S13 | built (acc-fe-admin) |
| `PATCH /agent-access/{id}` (name, purpose, team, departments, products; `If-Match`) | `admin-agent-access.html` Edit | tenant | `agent_access.manage` | yes | A; a stale version renders the 409 in place | ACC-S1 | built (acc-fe-admin) |
| `POST /agent-access/{id}/revoke` | `admin-agent-access.html` Revoke the agent | tenant | `agent_access.manage` | yes | A; every credential under it stops on the next call | ACC-S1, ACC-S13 | built (acc-fe-admin) |
| `PUT /agent-access/{id}/tenant-reach` (the entry's toggle) | `admin-agent-access.html` Our register | tenant | `agent_access.manage` | yes | A; disabled with its reason while the organisation's switch is off | ACC-S11, ACC-S13 | built (acc-fe-admin) |
| `POST /agent-access/{id}/keys`, `POST /agent-access/{id}/keys/{keyId}/revoke` | `admin-agent-access.html` Issue a key, Revoke | tenant | `agent_access.manage` | yes | A; the four read scopes only, expiry at most `AGENT_ACCESS_KEY_MAX_DAYS`, shown once | ACC-S1, ACC-S3 | built (acc-fe-admin) |
| `GET /agent-access/{id}/calls` | `admin-agent-access.html` Access log | tenant | `agent_access.manage` | no | A; paged; no question text or record content | ACC-S12 | built (acc-fe-admin) |
...
| Tenant reach: read the state (`GET /tenant/reach`) | `admin-security.html` panel; the reach line on `admin-agent-access.html` | tenant | `security.manage` (panel); `agent_access.manage` (the line) | no | A | ACC-S11 | built (acc-fe-admin) |
| Tenant reach: request, approve, reject (`POST /tenant/reach/requests`, `POST /tenant/reach/requests/{id}/approve`, `.../reject`) | `admin-security.html` panel | tenant | `security.manage` | yes | A, two different people; the requester's approval answers 409 `four_eyes_violation`, drawn as a state | ACC-S11, ACC-S13 | built (acc-fe-admin) |
| Tenant reach: switch off (`POST /tenant/reach/off`) | `admin-security.html` panel | tenant | `security.manage` | yes | A, one person; immediate for every entry | ACC-S11 | built (acc-fe-admin) |
