...
| `tenant-search.html` | moved into `/inventory` (D-9x) | tenant | 7 | superseded: the hit row reads in the inventory's search (SRC-S1, SRC-S3, SRC-S10); saved searches are chunk 13 | 16 / 15 |
| `tenant-ask.html` | `/ask` | tenant | 7 | shipped (SRC-S4, SRC-S5 and SRC-S10); its own page since D-9x |  |
...
| Operation | Card | Surface | Permission | Step-up | Role variants | E2E | Status |
|---|---|---|---|---|---|---|---|
| `POST /search` | `tenant-inventory.html` search bar | tenant | `search.use`; `key:search:read` | no | All 7 | SRC-S1, SRC-S3, SRC-S10, SRC-S14 | shipped; takes the obligations list's `instrument`, `term` and `footprint` since D-9x |
