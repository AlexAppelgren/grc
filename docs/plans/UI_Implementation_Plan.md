...
| 2026-09-25 | c8-cards-register: `tenant-gaps.html` and `tenant-obligation-units.html` drawn, and the register panels added to `tenant-obligation.html`, in both themes at 375 px and desktop. Applicability has no "Waiting for approval" and no step-up (D-75), so "Change waiting for approval" leaves the obligation row's slots in `pills-and-labels.md` and the inventory card; "Waiting for approval" stays for risk acceptance. The gap, units and register rows point at the cards | c8-cards-register |
| 2026-09-25 | c8-cards-admin: legal entities, licences and certificates, and products drawn on `admin-organisation.html`; the removal dialog on `admin-members.html`; `admin-support-access.html` and `console-support-access.html` drawn new. Every section in both themes at 375 px, no new tone | design agent |
...
| 2026-09-25 | c9-card-case-panels-b: the second half of the case panels drawn on `tenant-change.html`. Evidence (kind, name, size, who and when, the scan-state pill by kind, Download on a checked file only, a refused file still listed without a download; the attach dialog for a file, a link and a reference; uploading and "Being checked"; the 422 refusal quoting the allowed types and size limit from the server's answer; 503 when no scanner can check; remove with its confirm, soft), Sign-off (Request sign-off with its reason line from `canRequestSignoff` and `openActionCount` and the 409 `open_actions` and `evidence_missing` refusals in place; the waiting state naming who asked; Sign off and close with an optional note and the passkey prompt on 403 `step_up_required`; the requester's 409 `four_eyes_violation` in words; Send back with a required note), Closed (signed off with both names and final; closed without action or as not applicable with reason, who and Move back to triage), Dismissed (reason, who, Move back to triage), and Show case file with Export through the exports job, each with its permission-limited view, loading and error. Default taken: evidence is read-only while the case waits for sign-off, like actions, and Send back opens it again; the evidence and sign-off backend tasks confirm or overturn it. The screen-card row reads designed | design agent |
| 2026-09-25 | c10-cards-tagging: bulk tagging drawn on `tenant-inventory.html` (blocks 1 to 12: tenant tags on each row, the "Our tags" filter, row checkboxes and the page select-all, Tag for `vocab.manage`, the picker, the preview counts, the commit result, the cap refusal and the other answers, read only for everyone else) and the tags facet on `tenant-obligation.html` (the tags line split into library tags and "Our tags"; blocks "Our tags 1" to "Our tags 6": add, remove, Suggest without `vocab.manage`, none yet, the answers and the picker hints), in both themes at 375 px and desktop. Tenant tags stay an outlined information pill after the meta text, so no row slot and no tone is added. The inventory card gains the card bar and 375 px frames the other R2 cards carry | c10-cards-tagging |
| 2026-09-25 | acc-cards: `admin-agent-access.html` (the Access tab of Agents and the entry page) and `me-tokens.html` drawn; `admin-security.html` gains the tenant reach panel below the policy form, with its own actions outside Save; `admin-api-keys.html` is redrawn at the foundations' proportions as the list of every key and token with its kind and who it reads as. The Agent access operations below are mapped; paths the contract does not spell out yet are marked "path proposed" and fixed by the package that builds them (acc-entries-and-log, acc-scope-and-reach, acc-personal-grants). Defaults taken: the Access tab and entry pages need `agent_access.manage` to read as well as to write, since an entry exposes its credentials and access log; a reach request is decided only by a second person, so its requester has no Withdraw (the four-eyes CHECK covers rejection too); revoking one's own personal token needs no step-up, since it only takes access away. New pills are in `design/system/pills-and-labels.md` | acc-cards |
...
| `console-queue-batch.html` | `/console/queue/[proposalId]` (a batch proposal) | console | 11 | designed (c11-cards-agents-security): the batch header, before and after per row, row and whole-batch decisions with a reason from the vocabulary, step-up, the four-eyes refusal, the result with its audit link, and Ask for a re-tag (PRO-S8) |  |
...
| `console-languages.html` | `/console/languages` | console | 4 | not drawn: languages and jurisdictions are library vocabularies, and `admin-vocabulary.html`'s jurisdictions console variant (`/console/vocabularies/jurisdiction`, chunk 11, ADM-S18) is the card: a label per language, retire and restore by proposal, the mirrored scope term following, and the languages read-only |  |
...
| `tenant-gaps.html`; the register panels on `tenant-obligation.html` | `/gaps`, `/gaps/[gapId]`; `/inventory/obligations/[id]` | tenant | 8 | designed (c8-cards-register): the gap list with its filters, the record, Record a gap, Start remediation, Close, Reopen and risk acceptance on `tenant-gaps.html`; Does it apply to us, Where we stand, How we handle it, per legal entity, Gaps, Linked internal items and How we read this rule with the history on `tenant-obligation.html`. Cut from `vGaps()`, `vGap()`, `entityPanel()`, `gapsPanel()`, `historyPanel()`, `interpPanel()` |  |
| `admin-organisation.html` entities, licences and certificates, products, departments with heads, teams | `/admin/organisation` | tenant | 8 | extends the chunk 1 card; departments with heads and teams designed (f03-T60); legal entities as a tree, licences and certificates per entity (D-43, withdrawn rows dimmed) and products designed (c8-cards-admin, TEN-S2, TEN-S10) |  |
| `admin-members.html` team membership on the member row | `/admin/members` | tenant | 8 | extends the chunk 1 card; designed (f03-T60) |  |
| `admin-members.html` removal dialog with reassignment per kind | `/admin/members/[userId]` | tenant | 8 | designed (c8-cards-admin, TEN-S5): owned work by kind with counts, a new owner per kind, participations and teams, confirm with step-up, the `reassignment_required` and `unknown_member` refusals |  |
| `admin-support-access.html` | `/admin/support-access` | tenant | 8 | designed (c8-cards-admin, TEN-S6, TEN-S11; D-49, ADR 0042): pending requests with Approve (step-up) and Decline, the live grant with Revoke, history with the `support_access.read` rows, the read-only banner and the `support_read_only` and `support_access_ended` copy |  |
| `console-support-access.html` | `/console/support-access` | console | 8 | designed (c8-cards-admin, TEN-S6): request, own grants with their state, Enter with step-up; no member named, an approval shown as a time, Not found without a live grant |  |
...
| `tenant-obligation-units.html`: units list, paste dialog with its dry run, the confirmation of every decision and the Statement of Applicability; the Units panel on `tenant-obligation.html` opens it | `/inventory/obligations/[id]/units` (path proposed) | tenant | 8 | designed (c8-cards-register, REG-08) |  |
| case panels on `tenant-change.html` | `/watch/[changeId]` | tenant | 9 | designed (CAS-02 to CAS-08): triage, next step, impact assessment and actions (c9-card-case-panels-a); evidence, sign-off, closed and dismissed, and the case file with its export (c9-card-case-panels-b); every state at 375 px and on a desktop, light and dark, each panel naming its operation and permission; prototype `vChange()` work panels are the cut |  |
...
| `admin-workflow.html` | `/admin/workflow` | tenant | 10 | built by `c10-fe-workflow-policy` (COL-02, TEN-01; no scenario of its own); designed as: the six workflow fields with unit and platform default, saved, refused and restricted (CHUNK10_TASKS.md ruling 8: not a section of `admin-organisation.html`) |  |
...
| `admin-agents.html`, `console-agent-definitions.html` | `/admin/agents`, `/console/agents` | both | 11 | designed (c11-cards-agents-security): the bank's own agents with their controls, spend against one monthly cap, read-only What bleqq watches and the AI switch's state (AGT-S5, AGT-S13); the console's definitions, versions with publish and retire behind a passkey, platform settings and platform runs (AGT-S4) |  |
...
| bulk tagging on `tenant-inventory.html`; "Our tags" on `tenant-obligation.html` | `/inventory`; `/inventory/obligations/[id]` | tenant | 10 | designed (c10-cards-tagging, VOC-03, VOC-08): selection, Tag, preview, commit, cap refusal, the tenant tags on each row and the "Our tags" filter; the tags facet with the picker to add and × to remove for `vocab.manage`, read only with Suggest for everyone else |  |
...
| `admin-security.html` | `/admin/security` | tenant | 11 (moved from 13, CHUNK11_TASKS ruling 7); IP allow-list and SSO 13, retention 12 | designed (c11-cards-agents-security): session limits within the platform maximums, step-up on save (ID-S17); the passkey-policy section left the card when the device-bound policy moved out of R2 (D-100); the tenant reach panel (acc-cards): ask with a step-up, the waiting banner naming who asked, approve or reject with a passkey by a second person, the four-eyes refusal as a state, switch off (ACC-S11) |  |
| `admin-agent-access.html` | `/admin/agents/access`, `/admin/agents/access/[entryId]` | tenant | 11 | designed (acc-cards): the Access tab with its read-only line and the reach line; entries with state, reach and scope pills; register and edit with department, product and team pickers and a step-up; revoke; the entry page with credentials (issue with expiry at most 90 days, shown once, last use, revoke), the per-entry reach toggle disabled with its reason while the organisation's switch is off, and the access log with no content (ACC-S1, ACC-S3, ACC-S6, ACC-S12, J-11) |  |
| `me-tokens.html` | `/me/tokens` | tenant | 11 | designed (acc-cards): the member's own tokens with scopes, entry, expiry and last use; create with scopes backed by the member's permissions, an optional entry and a required expiry behind a step-up; shown once; revoke; the state without `tokens.create` (ACC-S3) |  |
| `admin-api-keys.html` (every key and token) | `/admin/api-keys` | tenant | 1; widened 11 | designed (acc-cards): the chunk 1 list, shipped, widened to every credential with its kind and who it reads as (an integration, an entry, a person), revoke for each; Create a key is unchanged (ACC-S3) |  |
...
| Operation | Card | Surface | Permission | Step-up | Role variants | E2E | Status |
|---|---|---|---|---|---|---|---|
| `PATCH /obligations/{id}/register` | `tenant-obligation.html` Where we stand, How we handle it, per legal entity | tenant | `register.edit` | no | CO, O edit; others read; `If-Match`, 409 stale_write rendered | REG-S3, REG-S4, REG-S11 | later chunk 8, designed (c8-cards-register) |
| `PUT /obligations/{id}/applicability` (per obligation, legal entity or unit; path proposed) | `tenant-obligation.html` Does it apply to us, with its confirmation dialog | tenant | `applicability.approve` | no (D-75) | CO, AP; others read; one audit event per write | REG-S1, REG-S2, REG-S4, REG-S12 | later chunk 8, designed (c8-cards-register) |
| `POST /applicability` (many rows in one call; path proposed) | `tenant-obligation-units.html` paste dialog, confirm the decisions | tenant | `applicability.approve` | no (D-75) | CO, AP; one audit event per row; `REGISTER_BULK_MAX` caps a call | REG-S14 | later chunk 8, designed (c8-cards-register) |
| `GET/POST` units and the paste with its dry run (paths proposed) | `tenant-obligation-units.html` units list and paste dialog | tenant | `register.edit`; `applicability.approve` when the paste carries applicability | no | CO, O | REG-S13, REG-S15 | later chunk 8, designed (c8-cards-register) |
...
| `GET/POST /obligations/{id}/internal-links`, `DELETE /internal-links/{id}` | `tenant-obligation.html` Linked internal items | tenant | `register.edit` | no | CO, O | REG-S8 | later chunk 8, designed (c8-cards-register) |
...
| `GET /agent-runs` | `console-health.html`; `admin-agents.html` history | both | `system.health`; `agents.manage` | no | PA; A | AGT-S5, ADM-S5 | later chunks 11 and 14; `admin-agents.html` designed, `console-health.html` card pending; the route itself is on `main` |
...
| Batch proposals (re-tag, backfill; approve whole or row by row) | `console-queue-batch.html` | console | `proposals.review` | yes | LE | PRO-S8, VOC-S12, AGT-S7 | later chunk 11, designed |
...
### Agent access (chunk 11, R2)

From `docs/plans/briefs/AGENT_ACCESS.md`, drawn by acc-cards. Every write on an entry asks
for a passkey. An agent access credential is read-only through R2 and reaches none of the
screen operations below; its own calls are the agent-only rows.

| Operation | Card | Surface | Permission | Step-up | Role variants | E2E | Status |
|---|---|---|---|---|---|---|---|
| `GET /agent-access`, `GET /agent-access/{id}` | `admin-agent-access.html` list and entry page | tenant | `agent_access.manage` | no | A; the tab is not offered without it, another tenant's entry is 404 | ACC-S1, ACC-S13 | later chunk 11, card designed |
| `POST /agent-access` (register) | `admin-agent-access.html` Register an agent | tenant | `agent_access.manage` | yes | A | ACC-S1, ACC-S13 | later chunk 11, card designed |
| `PATCH /agent-access/{id}` (name, purpose, team, departments, products; `If-Match`) | `admin-agent-access.html` Edit | tenant | `agent_access.manage` | yes | A; a stale version renders the 409 in place | ACC-S1 | later chunk 11, card designed |
| `POST /agent-access/{id}/revoke` (path proposed) | `admin-agent-access.html` Revoke the agent | tenant | `agent_access.manage` | yes | A; every credential under it stops on the next call | ACC-S1, ACC-S13 | later chunk 11, card designed |
| `PUT /agent-access/{id}/tenant-reach` (the entry's toggle; path proposed) | `admin-agent-access.html` Our register | tenant | `agent_access.manage` | yes | A; disabled with its reason while the organisation's switch is off | ACC-S11, ACC-S13 | later chunk 11, card designed |
| `POST /agent-access/{id}/keys`, `DELETE /agent-access/{id}/keys/{keyId}` (paths proposed) | `admin-agent-access.html` Issue a key, Revoke | tenant | `agent_access.manage` | yes | A; the four read scopes only, expiry at most `AGENT_ACCESS_KEY_MAX_DAYS`, shown once | ACC-S1, ACC-S3 | later chunk 11, card designed |
| `GET /agent-access/{id}/calls` | `admin-agent-access.html` Access log | tenant | `agent_access.manage` | no | A; paged; no question text or record content | ACC-S12 | later chunk 11, card designed |
| `GET /me/tokens`, `POST /me/tokens`, `DELETE /me/tokens/{id}` | `me-tokens.html` | tenant | any member (list and revoke their own); `tokens.create` (create) | yes (create) | A, CO, O create; AP, C, R, AU see the state without `tokens.create` | ACC-S3, ACC-S9 | later chunk 11, card designed |
| `GET /tenant/api-keys`, `DELETE /tenant/api-keys/{id}` (widened to every key and token) | `admin-api-keys.html` | tenant | `integrations.manage` | no | A; kind and who it reads as on every row | ACC-S3 | later chunk 11, card designed |
| Tenant reach: read the state (`GET /tenant/reach`, path proposed) | `admin-security.html` panel; the reach line on `admin-agent-access.html` | tenant | `security.manage` (panel); `agent_access.manage` (the line) | no | A | ACC-S11 | later chunk 11, card designed |
| Tenant reach: request, approve, reject (`POST /tenant/reach/requests`, `POST /tenant/reach/requests/{id}/approve`, `.../reject`, paths proposed) | `admin-security.html` panel | tenant | `security.manage` | yes | A, two different people; the requester's approval answers 409 `four_eyes_violation`, drawn as a state | ACC-S11, ACC-S13 | later chunk 11, card designed |
| Tenant reach: switch off (`DELETE /tenant/reach`, path proposed) | `admin-security.html` panel | tenant | `security.manage` | yes | A, one person; immediate for every entry | ACC-S11 | later chunk 11, card designed |
| `POST /agent-access/what-applies` | none | agent | `key:library:read` (register rows need `key:tenant:read` and reach) | no | agent access credentials | ACC-S5, ACC-S6, ACC-S13 | agent only |
| MCP server (`search`, `list_obligations`, `get_obligation`, `list_upcoming_changes`, `list_register_entries`, `what_applies`) | none | agent | the scope of the REST route behind each tool | no | agent access credentials; a write answers 403 `read_only_credential` | ACC-S7, ACC-S13 | agent only |
