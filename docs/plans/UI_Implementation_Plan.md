...
| 2026-09-25 | c9-fe-signoff-casefile: the Sign-off and Case file panels built on `tenant-change.html` (`SignoffPanel`, `CaseFilePanel`, CAS-06, CAS-07). Request sign-off needs `cases.work` and reads `canRequestSignoff` and `openActionCount` for its reason line; a stale tab's 409 `open_actions` quotes the count from the answer and `evidence_missing` asks for checked evidence, in place. Waiting names who asked and when; Sign off and close with an optional note and the shared passkey prompt, the requester's 409 `four_eyes_violation` in words; Send back with a required note. A signed-off case shows both names with their dates. Show case file, for `cases.read`, reads the server's text and shows it in its sections with each SHA-256 in monospace; Export, only with `exports.create`, follows the `case_file` job to its download. Departures from the card: Request sign-off is disabled while `canRequestSignoff` is false rather than left enabled (the package's acceptance), so the 409s show only from a stale tab; the signed-off panel shows no note, because the change page's case block carries none (the case file prints it); Show case file sits in a panel of its own in every status rather than in the Closed and Dismissed bars | c9-fe-signoff-casefile |
| 2026-09-25 | c9-fe-triage-assessment: the Triage and Impact assessment panels built on `/watch/[changeId]` (`tenant-change.html`). Triage with the owner picker from `GET /reference/people?permission=cases.work` and the bank's urgency list, the Dismiss dialog, Next step with Start assessment and No action (close reasons of the `no_action` kind, optional note), Dismissed and Closed (one person) with Move back to triage when `allowedTransitions` holds `new`; the assessment form with the effort and sub-status pickers, "No" offered only to `cases.work` while assessing and confirmed before it saves, read-only without `cases.contribute` and from waiting for sign-off. A 422 renders under the field its `errors` names, `stale_write` keeps the person's input and offers a reload. Departures: no owner-team picker (c9-owner-team-and-reassign adds `ownerTeamId`), no contributor teams (c9-fe-case-participants), and the closed panel names the date but not the person, since the workflow block carries no closer | c9-fe-triage-assessment |
| 2026-09-25 | c9-fe-actions-evidence: the Actions and Evidence panels built on `tenant-change.html` (CAS-04, CAS-05). Actions: owner, due date, days left and late in tabular numerals, Overdue as a negative pill from the tenant-local day, the add row and Remove with its confirm for `cases.work`, the completion box for `cases.contribute`; locked with its reason while the case waits for sign-off and read-only once closed; `actions_locked` and `stale_write` render in place with a reload, and the cap's 409 in the server's words with its number. Evidence: the scan-state pill by kind with the reason a file cannot be opened, Download on a checked file only for `cases.read`, the attach dialog for a file (one multipart post with its progress), a link (opened only through `externalHref`) and a reference, the server's refusals quoted (allow-list and size, the evidence cap), 503 in words, Remove with its confirm. A download is bytes through the API saved under the server's `Content-Disposition` name, so `content-disposition` joins `CORS_EXPOSE_HEADERS`; nothing is kept in storage, the address bar or a log. Defaults taken: the add row has no owner picker, since `GET /reference/people` is not on this branch's base, so a new action is the case owner's (the contract's own default) and the row says so; the picker joins when the people read lands. A file row reads "File" and its size without the type the card shows, which would need a client copy of the allowed types. Send as tickets is chunk 13's | c9-fe-actions-evidence |
