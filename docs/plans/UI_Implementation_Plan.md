...
| 2026-09-25 | c9-fe-case-participants: the Participants panel built on `/watch/[changeId]` (`tenant-change.html`): the owner read-only above the list, who added each row, one picker over `GET /reference/people?permission=cases.read` and the bank's teams, Remove and Add with `cases.contribute`, Leave on one's own row, and every refusal (a closed or dismissed case's `invalid_transition` included) in place by its code. The assessment's Contributor teams are the case's team participants as outlined `information` pills (a bank's own list row, like a tenant tag), each added from a teams-only picker or removed with one call of its own, never part of the save (D-20), and read in the read-only assessment. Departure: no owner team, which the case does not carry until c9-owner-team-and-reassign | c9-fe-case-participants |
