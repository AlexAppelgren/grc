...
| case panels on `tenant-change.html` | `/watch/[changeId]` | tenant | 9 | card in progress: triage, next step, impact assessment and actions designed in every state, each naming its operation and permission (c9-card-case-panels-a); evidence, sign-off, closed and dismissed and the case file pending (c9-card-case-panels-b); prototype `vChange()` work panels are the cut |  |
