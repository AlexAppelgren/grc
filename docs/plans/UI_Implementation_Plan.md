...
| 2026-09-25 | c8-ui-gaps-risk: `tenant-gaps.html` built at `/gaps` and as the Gaps panel on `tenant-obligation.html`. There is no `GET /gaps/{id}`, so the record opens in place under its row on both, not at `/gaps/[gapId]`. Status, severity, owner ("Me") and target-date filters in the URL; no legal-entity filter and a person-or-team owner picker limited to "Me" and the teams until a people and organisation reference every member may read exists. The list opens on every status, since the API filters by one key. Withdraw and Reject are drawn but have no route, so neither is offered. REG-S5 (to the roadmap step) and REG-S6 un-fixmed | c8-ui-gaps-risk |
| 2026-09-25 | c8-ui-home-register: "Where we stand" built on Today (`tenant-today.html`) beside "Decide now": the obligations that apply, a bar and a line per compliance category in its tone, the open gaps and "See the gaps". Each category opens `/inventory?applicability=applies&complianceStatus=<key>` through the bank's system status of that category, and the gaps `/gaps` unfiltered, since the count spans two statuses (both in TODO_FOR_alex); an empty register shows a sentence and the way to the inventory. The roadmap (`tenant-roadmap.html`) names each of our own deadlines on its card ("Our deadline · Next review · Johan Berg") and in its detail (what, owner, legal entity, "Open obligation"); "Add to calendar" and "So what?" on the design's detail are not built, since a feed never carries our own dates (D-52) and no internal item has a So what. HOM-S15 un-fixme'd | c8-ui-home-register |
