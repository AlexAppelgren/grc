...
| `admin-organisation.html` entities, licences and certificates, products, departments with heads, teams | `/admin/organisation` | tenant | 8 | extends the chunk 1 card; departments with heads and teams designed (f03-T60); legal entities as a tree, licences and certificates per entity (D-43, withdrawn rows dimmed) and products designed (c8-cards-admin, TEN-S2, TEN-S10) ; departments with heads and teams shipped (c8-ui-departments-teams-removal, TEN-S8) |  |
| `admin-members.html` team membership on the member row | `/admin/members` | tenant | 8 | extends the chunk 1 card; designed (f03-T60); shipped on the member screen (c8-ui-departments-teams-removal, TEN-S8); the member list does not show teams yet |  |
| `admin-members.html` removal dialog with reassignment per kind | `/admin/members/[userId]` | tenant | 8 | designed (c8-cards-admin, TEN-S5): owned work by kind with counts, a new owner per kind, participations and teams, confirm with step-up, the `reassignment_required` and `unknown_member` refusals ; shipped (c8-ui-departments-teams-removal): Deactivate on the member screen opens it |  |
...
| `PUT /tenant/members/{userId}/teams` | `admin-members.html` member row | tenant | `members.manage` | no | A | TEN-S8 | shipped (c8-ui-departments-teams-removal) |
