...
| 2026-09-25 | Alex (D-103): the inventory's row of selects did not work. `inventory-search-filters`: one search bar and a Filters sheet on `/inventory` (`tenant-inventory.html`); the sheet holds Instrument (searchable, with counts), Regime, Service, Duty type and As of, set filters show as removable chips, and Regime and Service offer only the scope's terms. Scope is a segmented control on the tab row: My scope, Markets we watch, Show all items. The bar runs `POST /search` over obligations and provisions inside the same filters and scope (the search now takes the list's own `instrument`, `term` and `footprint`); on the Instruments tab it runs `GET /instruments?q`. `/search` is gone: Ask moved to `/ask` and the nav item reads "Ask". Team scope and the withheld register overlay are R2 (TEN-03, chunk 8) |
...
| `tenant-search.html` | moved into `/inventory` (D-103) | tenant | 7 | superseded: the hit row reads in the inventory's search (SRC-S1, SRC-S3, SRC-S10); saved searches are chunk 13 | 16 / 15 |
| `tenant-ask.html` | `/ask` | tenant | 7 | shipped (SRC-S4, SRC-S5 and SRC-S10); its own page since D-103 |  |
...
| Operation | Card | Surface | Permission | Step-up | Role variants | E2E | Status |
|---|---|---|---|---|---|---|---|
| `POST /search` | `tenant-inventory.html` search bar | tenant | `search.use`; `key:search:read` | no | All 7 | SRC-S1, SRC-S3, SRC-S10, SRC-S14 | shipped; takes the obligations list's `instrument`, `term` and `footprint` since D-103 |
