...
- `c4-close-a` (`chunk4-T24a`) moves to the end of the chunk's waves and records that PRO-S12's
...
- **Chunk 11 (a note, not a package).** `c11-agent-definitions-contract` versions the confirming
  agent's definition like any other platform definition, and
  `c11-fe-console-agent-definitions` shows what it decided. Chunk 11 has no task file yet;
  `docs/plans/briefs/CHUNK11_TASKS.md` holds the note until it is planned.
