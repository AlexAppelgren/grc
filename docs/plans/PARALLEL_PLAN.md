...
5. **Shared files come in three classes.**
   ...
   - **Append ledgers**: several running packages may add to them. Each adds only its own rows, entries, test blocks, test classes or settings banner, and never edits, moves or deletes another's lines. The later merge takes the union inside its own worktree and reruns its gates. The list:
     ...
     - `frontend/src/shared/i18n/messages.ts`, where a package adding a message namespace appends only its own two import lines and its own two spread entries, both in alphabetical order (rule 6);
...
6. **The frontend split comes before the frontend fans out.**
   - `x-frontend-split` waited for c4-console-shell, which edited both catalogs. It moved `frontend/src/messages/en.json` and `sv.json` into `frontend/src/messages/<namespace>/en.json` and `sv.json`, one pair per feature namespace, and taught `check:messages` and `check:copy-drift` to read the directory. Landed 2026-09-20 with twelve namespaces: `auth`, `common`, `console`, `dev`, `footprint`, `inventory`, `library`, `me`, `nav`, `tenant-admin`, `today` and `vocabularies`; `c5-fe-copy-nav` then added `watch`, `console-change-facts`, `console-sources` and `console-agent-keys` for the chunk 5 screens.
   - Each namespace pair belongs to the key of the feature that owns it (section 3.4). A package that needs a namespace nobody has created yet adds the pair, its two import lines and its two spread entries in `frontend/src/shared/i18n/messages.ts`. That file is an append ledger under rule 5, so two packages may each add their own namespace at the same time; a package never edits another's lines, and a pair nobody imported fails `npm run typecheck` the moment a screen asks for one of its keys, because `MessageKey` is the merged catalog.
