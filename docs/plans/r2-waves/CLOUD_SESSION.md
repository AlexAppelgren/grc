...
## Decisions
Take the docs/DECISIONS.md default. D-74, D-75 and D-89 are Alex's decisions; D-89 means agents never manage a bank's regulatory scope and scope changes keep four eyes. The plan pre-assigns D-91 to D-97 to the packages its briefs name. If you need another decision row, number it D-9x with your package key in the row; the integrator renumbers. Anything that genuinely needs Alex goes in docs/TODO_FOR_alex.md in a block headed with your package key, with the default you took.
