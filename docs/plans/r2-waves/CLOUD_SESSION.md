...
- Two gates belong to the integrator, and only these may stay red when you push your done marker: prepush's "OpenAPI/TS: artefacts match the contract", when the only drift is the regeneration of your own routes and schemas (you regenerate to build and typecheck but never commit the two files), and the demo check in `demo.journey.spec.ts` that asks for `npm run demo:record`. Name both in your report. Every other gate must be green.
- prepush's gitleaks scans every origin branch with your checkout's config, so another package's new test key can flag a commit that is not in your history. That is not yours: run gitleaks on your own range (`gitleaks git . --config .gitleaks.toml --log-opts="origin/main..HEAD"`), and if that is clean, run the gates after gitleaks by hand and treat gitleaks as green; name the foreign commits in your report. Never delete, rewrite or push other branches' refs to get past it. If your own package adds an E2E test key, allowlist it by exact literal and path in .gitleaks.toml as playbook 8.3 says.
