...
| 3 | Library and inventory | R1 | | | | in progress | **Implemented and Tested are empty on purpose** (review of `645b1c3..2492f80`): the chunk's own requirement list is not all on `main` — INV-08 (the first standard), FP-04's chunk-3 part (the markets panel on Regulatory scope, FP-S10, and the inventory's watched-market view, FP-S13; the feed's view is chunk 5's) and FP-01's opt-in rule and regime fold are pending — and INV-S11 and INV-S12 are skipped, FP-S4's and INV-S14's journeys are `test.fixme`, and the screen does not yet render a machine-confirmed record as such. The data layer, the FFFS 2017:2 provision tree seed, the instrument and provision reads and the inventory and instrument screens are on `main`: instruments and provisions with identity, dates and lineage (INV-01), a three-level provision tree with verbatim versions and transitional notes (INV-02), obligations with facets and diffs (INV-03, INV-04, unchanged from earlier chunk work), original-language text with labelled machine translations on both obligations and provisions (INV-05), a source link, a last-verified date and "this looks wrong" on both instruments and obligations, provisions reporting through their instrument (INV-06). The one footprint rule (`reading.instrument_scopes()`, `reading.obligation_scopes()` inheriting through it) now covers instruments as well as obligations, and the inventory's Instruments tab carries its own "Show outside our scope" switch (FP-03). Cut: `GET /instruments/{id}/problem-reports` and `GET /obligations/{id}/problem-reports` as list endpoints (a reader files a report but nobody reads the list back yet; moves to chunk 4 with the tenant's own proposal queue), `GET /authorities` as a standalone route (re-annotated to chunk 5; an instrument's authority is read embedded today), FP-S4's report surfaces (no register exists yet; chunk 12), the vocabulary screen's real usage count for `relation_type` (stays hardcoded 0) and its merge re-pointing (`repoint.py` has no `relation_type` function, so merging two relation types would not move `instrument_relation` rows off the retired key; both moves to whichever chunk next touches `relation_type`, not INV-01 or INV-02 work). INV-S14's integration half landed in 2492f80 (a record an agent proposed and another agent confirmed carries machine-confirmed provenance naming both); its journey and the screen's machine-confirmed rendering remain. INV-07 (tenant-private library records) and INV-08 (standards) stay pending, R3 and PRD 0.3 respectively |
| 4 | Proposals and the platform console | R1 | | | | in progress | Built: the console shell and its own sign-in, the proposal queue and one proposal's screen with four eyes and step-up, the rejection-reason list, the tenant's own request list, the audit read for a bank and for the library, the console's tenants, vocabularies, sources, change-facts and agent-key screens, and the tenant's own "what changed in the library while we were away" (PRO-S7, `GET /library-updates`), and the independent agent as the second principal on a proposal: the integration halves of PRO-S13, PRO-S14, ID-S31, AUD-S9 and INV-S14 landed in 2492f80. Not yet: PRO-S13's and INV-S14's journeys and the machine-confirmed rendering on screen; PRO-S7's journey; AUD-03, a problem report kept inside the bank that filed it, with the bank's own list and close (AUD-S5 is skipped and its journey fixme; filing a report is built); AGT-S15, the confirming agent's independence as one scenario, with its run recording the proposals it decided; PRO-S10 and PRO-S11, the proposal rules for a standard, which wait on INV-08; and the platform agent-key routes, which still answer 501 (ID-S20). 7224bb8 (the earlier `r1-agent-curation` attempt) is superseded by 2492f80, except its `agent_not_bound` refusal and `is_mine`, which are being ported. Not this chunk's: PRO-04 batch proposals (PRO-S8) are chunk 11, and ADM-02's system health (ADM-S5) is chunk 14 |
| 5 | Watch and the agent API | R1 | | | | in progress | Built: sources and their checks, change registration and curation through the agent key, the watch feed and one change's page with its timeline, documents and obligation links, the AI-drafted "So what?" per tenant, the fan-out that opens a case in every bank marked inside or outside its scope, and the re-check when the scope moves, and the coverage tab: `GET /sources/coverage` is served and WAT-S1 is green. Not yet: a standard's new edition as one change (WAT-S10); every change carrying a regime, a standard term needing a standards body and a publisher's page keeping no snapshot (WAT-S11); the library re-check that proposes a correction (WAT-S12); a run from open to close as one scenario and the vocabularies read at run start (AGT-S1, AGT-S3, both skipped); out-of-scope documents counted and never registered, behind the eval gate (AGT-S12); a change's jurisdiction from its authority and the feed's watched-market view (FP-S15, FP-04's chunk-5 part); the "So what?"'s Rewrite and Confirm wording on screen (WAT-S7's journey is fixme); J-4 end to end (AGT-S10's journey is fixme); and curation confirmation. D-74 answered who confirms (an agent of a different definition and key, labelled machine-confirmed), and none of it is built: there is no confirm route, `change_term` and `change_obligation` have no agent columns and their `confirmed_by` is a user, `regulatory_change.change_type` has no `suggested` or `confirmed_*` columns, and the console's Change facts detail has no Confirm control, so WAT-S4 and the library half of WAT-S6 wait on it. Not this chunk's: WAT-06 tenant source requests (WAT-S8) are R3, chunk 13 |
...
| 7 | Search and ask | R1 | | | | in progress | Built: the hybrid index and its rebuild inside an approval, search by reference and by idea with one ranked answer, similar records, the per-caller rate limits and the search screen. Ask's backend (from the groundwork of a86b3dd on `claude/r1-ask`): `POST /ask` streams an answer grounded in the reader's own ranking on the mock model, one cited statement at a time, flags a confirmed pending change with the day it takes effect, answers "no answer" without a model call, refuses before the stream for the bank's AI switch (`tenant.ai_enabled`, 403 `feature_off`), the rate limit and the language, and logs every model call to the AI log with how it ended, even when the reader leaves early or the model fails part way; SRC-S4, SRC-S5 and SRC-S6 pass. Not yet: the Ask panel and the SRC-S4, SRC-S5 and J-7 (SRC-S10) journeys, the reader's verdict (`rateAnswer`) and SRC-S9's budgets, and the route that switches AI off. First test deploy target after this chunk |
