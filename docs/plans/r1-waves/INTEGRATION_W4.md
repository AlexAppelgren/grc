...
6. **Agent-approvable kinds.** main now has `AGENT_CONFIRMABLE_KINDS` = obligation version, vocabulary kinds, term kinds, `new_instrument` and `new_obligation` (their applied rows record `verified_origin` and `verified_by_agent`); `new_provision` and `new_provision_version` stay person-only. security-review-c4 was written before that and its low row L15 says the instrument and obligation kinds would pass CI "if added by mistake": keep main's set, reword that HARDENING row to say the widening is deliberate under D-79 and that the provision kinds are the ones a test must pin refused, and add that test (an agent approving `new_provision_version` answers 409 `person_review_required`) if it is missing.
7. **Perf baseline.** If a merge changes a measured route's queries, re-run `perf_report` for it and keep backend/perf/baseline.json consistent; no route may exceed its budget.
