...
| D-62 | Who gives the second pair of eyes, and the shape of it | `docs/DECISIONS.md` | — |
| ADR 0054 | The second pair of eyes on a library proposal may be an independent agent | `docs/adr/0054-agent-approves-from-the-same-queue.md`, indexed in `docs/adr/README.md` | — |
| PRO-S13 | An independent agent confirms a proposal from the same queue (`@integration` `@e2e`) | `proposals/app.md` | `chunk4-T25` (integration); the journey waits for chunk 5 |
| PRO-S14 | The same principal can never both propose and approve (`@integration`) | `proposals/app.md` | `chunk4-T25` |
| INV-S14 | A record an agent confirmed reads as machine-confirmed (`@integration` `@e2e`) | `library/app.md` | `chunk4-T26`; the journey waits for chunk 5 |
| AGT-S15 | The confirming agent is independent of the proposing agent (`@integration`) | `agents/app.md` | chunk 5, with the confirming agent's definition |
| AUD-S9 | An agent's approval is in the audit trail with the agent named (`@integration`) | `governance/app.md` | `chunk4-T25` |
| ID-S31 | The review scope reaches the queue and never a library row (`@integration`) | `identity/app.md` | `chunk4-T25` |
...
| `proposals:review` | The new platform-only API key scope | PRD §6, `INPUT_DELTAS.md` §2, ADR 0054 | `chunk4-T25` |

Each id is the next free number in its own `app.md` on `main`, checked one app at a time rather
than assumed: `agents/app.md` already carries AGT-S13 and AGT-S14, `governance/app.md` AUD-S8,
`identity/app.md` up to ID-S30, `library/app.md` INV-S13 and `proposals/app.md` PRO-S12. Two plan
files reserve ids that `agents/app.md` has since used for other scenarios (`CHUNK5_TASKS.md` holds
AGT-S13 for `c5-library-recheck`, `CHUNK11_TASKS.md` holds AGT-S14 for
`c11-platform-agent-settings-a`); that collision predates this decision and is left for the
sessions that own those plans.
...
- `docs/DECISIONS.md` — D-62 added; D-14 narrowed to point at it; the closing note names ADR 0054.
- `docs/adr/0054-agent-approves-from-the-same-queue.md` — new. `docs/adr/README.md` — its row and
  the PRD 0.4 note.
...
- `backend/apps/proposals/app.md` (context, AC-PRO2, PRO-S13, PRO-S14) and `tests_scenarios.py`.
- `backend/apps/library/app.md` (context, INV-S14) and `tests_scenarios.py`.
- `backend/apps/agents/app.md` (context, AGT-S15) and `tests_scenarios.py`.
- `backend/apps/governance/app.md` (context, AUD-S9) and `tests_scenarios.py`.
- `backend/apps/identity/app.md` (context, ID-S31) and `tests_scenarios.py`.
- `frontend/tests/e2e/proposals.journey.spec.ts` (PRO-S13) and `library.journey.spec.ts` (INV-S14).
...
2. **Four eyes is widened in the database, not in Python alone.** `proposal` gains
   ...
   a repeated user, key or agent, and a reviewing key that names no agent at all. The agent
   columns are denormalised copies of each key's agent, because a CHECK cannot dereference a
   foreign key and two keys of one definition must not be able to confirm each other; the
   not-null clause is what stops two unbound keys passing the agent comparison on nulls.
...
8. **J-4 is unchanged** — the golden path still shows a person approving in the console, because
   that path is built and tested end to end. `PRO-S13` is the agent path, and its journey half
   waits for chunk 5.
...
- PRO-S14's Gherkin: it is the scenario that has to fail if the widened constraint is ever
