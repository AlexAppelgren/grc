...
> design for PRD module **ACC** (ACC-01 to ACC-10), decisions **D-63 to D-68** and
> ADRs **0055, 0056 and 0057**. The PRD wins on any conflict; this file is the
> detail an implementing agent needs and the PRD does not carry.
...
1. **Both credential kinds, service key first** (D-64). A service key bound to
   an entry, and a personal access token that acts as a person.
2. **The bank's own register is the payload** (D-63). "The agent can access
...
3. **Scope comes from departments and products** (D-65), which the bank already
   maintains under TEN-02. No second scope list to keep current.
4. **Chunk 11, R2** (D-68), beside the tenant agent work that already builds
   credentials, budgets and the Agents screen.
5. **A summary and the full list** (D-66). "I want both a short summary of what
   applies but also a full list of registries that applies."
6. **A tenant-wide switch plus per-entry toggles** (D-67) for letting tenant
...
| `tenant_reach` | Boolean, default false. The per-entry half of D-67 |
...
1. **A short summary**, drafted by our model, carrying the AI-drafted label and
   ...
   approved applicability is the decision.
...
Two switches, because D-67 asks for both and they do different jobs.
...
- **REG-04 should be promoted** above the descope line in chunk 8. "How we read
  ...
  that protects work, so reordering chunk 8 is Alex's call and it is in
...
## 14. Open, and in `docs/TODO_FOR_alex.md`
...
- **REG-04's promotion** in chunk 8 (section 10).
- **Where a bank draws the line on its own model.** We say a bank pulling its own
  ...
  procurement review will ask what we do about it. Proposed: we state it plainly
...
- **Whether an auditor's agent is a tenant entry or something else.** An external
  ...
  exactly, but it is a third party inside the bank's tenant. Proposed: the bank
  registers it like any other entry, under the auditor's named engagement, and the
  entry's expiry does the work. TEN-06's support access grants are the nearer
  pattern if that is wrong.
