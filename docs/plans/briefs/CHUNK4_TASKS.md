...
PRO-S13's E2E half is not this task's: it waits for the confirming agent's definition in chunk 5,
...
**Scenarios:** PRO-S13 (`@integration` half, un-skipped), PRO-S14, ID-S31, AUD-S9 (all un-skipped); PRO-S2, PRO-S5 and ID-S21 stay green
...
- **The constraint.** `proposal_four_eyes` is replaced, in the same migration, by a constraint
  that refuses a row whose user, key **or** agent is the same on both sides, and that also refuses
  a reviewing key naming no agent: `reviewed_by_api_key IS NULL OR reviewed_by_agent IS NOT NULL`.
  Without that clause two unbound platform keys would both carry a NULL agent, and `NULL <> NULL`
  is unknown, so the agent comparison would pass on a missing value rather than on independence.
  The clause belongs in the constraint, not only in the fence guard, because the database is what
  answers when the logic is bypassed. It is proven to fail once by inserting each of the three
  repeats and once by inserting a reviewing key with no agent, then restored. The 409
...
- `backend/apps/identity/tests_scenarios.py` (the ID-S31 test only)
- `backend/apps/governance/tests_scenarios.py` (the AUD-S9 test only)
- `backend/apps/shared/factories.py` (a platform key bound to an agent, for the fixtures)

**Done when:**

- PRO-S13's integration half, PRO-S14, ID-S31 and AUD-S9 are un-skipped and green, written before
  the code.
...
- A direct insert repeating the user, the key or the agent is refused by the constraint, and so
  is one whose `reviewed_by_api_key` is set with `reviewed_by_agent` null.
...
**Scenarios:** INV-S14 (un-skipped); INV-S7 and INV-S8 stay green
...
- INV-06's re-verification stamp is untouched and stays a person's act writing `verified_by`; a
  later stamp supersedes the machine-confirmed label, which INV-S14 pins.

**Owned paths:**

- `backend/apps/library/models.py`, `migrations/`
- `backend/apps/library/reading.py`, `schemas.py` (the provenance fields only)
- `backend/apps/library/tests_scenarios.py` (the INV-S14 test only)
- `backend/apps/proposals/apply.py`

**Done when:**

- INV-S14 is un-skipped and green, written before the code.
...
`PRO-S13` is `@integration` and `@e2e`. Its integration half lands here, with a platform key
